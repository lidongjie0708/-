...
class Settings:
    ...
    evaluator_llm_model: str = os.getenv("RAGAS_EVALUATOR_LLM_MODEL", "deepseek-chat")
    evaluator_embedding_model: str = os.getenv("RAGAS_EVALUATOR_EMBEDDING_MODEL", "text-embedding-v3")
    evaluator_max_tokens: int = _int("RAGAS_EVALUATOR_MAX_TOKENS", 384000)
    ...
    rag_vector_top_k: int = _int("RAG_VECTOR_TOP_K", 20)
    rag_bm25_top_k: int = _int("RAG_BM25_TOP_K", 20)
    rag_rrf_k: int = _int("RAG_RRF_K", 60)
    rag_rerank_top_k: int = _int("RAG_RERANK_TOP_K", 6)
